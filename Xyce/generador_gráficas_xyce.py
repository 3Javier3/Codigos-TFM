...
try:
    # 1. Cargar datos (ignorando el texto informativo del final de Xyce)
    ...
    print(f"--- Resultados de la Célula ---")
    print(f"Voltaje MPP: {v_mpp:.4f} V")
    print(f"Corriente MPP: {i_mpp:.4f} A")
    print(f"Potencia Máxima: {p_max:.4f} W")
    ...
    # Título y Leyendas
    plt.title('Curvas Características I-V y P-V (Simulación Xyce)', fontsize=14)
    lines = line1 + line2
    labels = [l.get_label() for l in lines]
    ax1.legend(lines, labels, loc='center left')

    plt.tight_layout()
    plt.savefig('grafica_solar_completa.png', dpi=300)
    plt.show()
